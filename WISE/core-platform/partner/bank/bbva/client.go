...
func newHTTPClient(proxyConfig *HttpProxyConfig) *retryhttp.Client {
	client := retryhttp.NewClient()
	client.Logger = nil
	if proxyConfig != nil {
		certs := x509.NewCertPool()
		certs.AppendCertsFromPEM(proxyConfig.CACertData)
		tlsConfig := &tls.Config{
			RootCAs: certs,
		}
		var transport *http.Transport = &http.Transport{
			TLSClientConfig: tlsConfig,
			Proxy:           http.ProxyURL(proxyConfig.ProxyUrl),
...
var httpClientWithProxy *retryhttp.Client = nil

func (s *config) getHttpClientWithProxy(proxyConfig *HttpProxyConfig) *retryhttp.Client {
	if httpClientWithProxy == nil {
		httpClientWithProxy = newHTTPClient(proxyConfig)
	}
...
func (s *client) do(req *retryhttp.Request, response interface{}, proxyConfigs ...HttpProxyConfig) error {
	...
	if len(proxyConfigs) != 0 {
		...
		httpResp, err = s.getHttpClientWithProxy(&proxyConfig).Do(req)
	} else {
...
func (s *client) FetchVGSProxyConfig() HttpProxyConfig {
	httpsProxyUrl := os.Getenv("VGS_HTTPS_PROXY_URL")
	proxyUrl, err := url.Parse(httpsProxyUrl)
	if err != nil {
		log.Fatalln("Invalid VGS_HTTPS_PROXY_URL " + httpsProxyUrl)
	}
	data := os.Getenv("VGS_CERT")
	if len(data) == 0 {
		log.Fatalln("Failed to fetch vgs certificate")
	}
	return HttpProxyConfig{
		ProxyUrl:   proxyUrl,
		CACertData: []byte(data),
	}
}
...
func handleAPIResponse(resp *clientResponse, httpResp *http.Response) error {
	...
	if httpResp.StatusCode >= 400 {
		...
		if os.Getenv("DEBUG") == "" {
			dump, _ := httputil.DumpResponse(httpResp, true)
			log.Println("Error Response: ", string(dump))
		}
