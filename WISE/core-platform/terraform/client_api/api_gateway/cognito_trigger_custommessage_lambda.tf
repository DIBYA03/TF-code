resource "aws_lambda_function" "cognitoauth_custommessage_lambda" {
  function_name = "${module.naming.aws_lambda_function}-cognitoauth-custommsg"
  role          = "${aws_iam_role.cognito_lambda_default.arn}"
  timeout       = "${var.lambda_timeout}"
  kms_key_arn   = "${aws_kms_alias.cognito_lambda.target_key_arn}"

  filename         = "../../../cmd/lambda/cognitoauth/custommessage/lambda.zip"
  source_code_hash = "${base64sha256(file("../../../cmd/lambda/cognitoauth/custommessage/lambda.zip"))}"
  runtime          = "go1.x"
  handler          = "main"
  ...
  environment {
    variables = {
      ...
      AUTH_SMS_MESSAGE   = "${var.cognito_sms_prepend_message}"
      FORGOT_SMS_MESSAGE = "${var.cognito_sms_prepend_message}"
      RESEND_SMS_MESSAGE = "${var.cognito_sms_prepend_message}"
      SIGNUP_SMS_MESSAGE = "${var.cognito_sms_prepend_message}"
      ...
      REDIS_URL      = "${data.aws_ssm_parameter.redis_endpoint.value}"
  ...
  tags {
    ...
    Name        = "${module.naming.aws_lambda_function}-cognitoauth-custommsg"
    Team        = "${var.team}"
  }
}

# Give permissions for Cognito to access the lambda function
resource "aws_lambda_permission" "cognitoauth_custommessage_lambda" {
  statement_id  = "AllowExecutionFromCognito"
  action        = "lambda:InvokeFunction"
  function_name = "${aws_lambda_function.cognitoauth_custommessage_lambda.function_name}"
  principal     = "cognito-idp.amazonaws.com"
  source_arn    = "${aws_cognito_user_pool.default.arn}"
}

resource "aws_cloudwatch_metric_alarm" "cognitoauth_custommessage_lambda_non_critical_errors" {
  alarm_name          = "${module.naming.aws_lambda_function}-cognitoauth-custommsg-non-crit-errors"
  ...
  dimensions {
    FunctionName = "${aws_lambda_function.cognitoauth_custommessage_lambda.function_name}"
  }
  ...
  tags {
    ...
    Component   = "${var.component}"
    Environment = "${var.environment_name}"
    Name        = "${module.naming.aws_lambda_function}-cognitoauth-custommsg-non-crit-errors"
    Team        = "${var.team}"
  }
}

resource "aws_cloudwatch_metric_alarm" "cognitoauth_custommessage_lambda_critical_errors" {
  alarm_name          = "${module.naming.aws_lambda_function}-cognitoauth-custommsg-crit-errors"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "2"
  metric_name         = "Errors"
  namespace           = "AWS/Lambda"
  ...
  tags {
    ...
    Component   = "${var.component}"
    Environment = "${var.environment_name}"
    Name        = "${module.naming.aws_lambda_function}-cognitoauth-custommsg-crit-errors"
    Team        = "${var.team}"
  }
}

resource "aws_ssm_parameter" "cognitoauth_custommessage_lambda_warmer" {
  name      = "/${var.environment}/${aws_lambda_function.lambda_warmer_lambda.function_name}/${aws_lambda_function.cognitoauth_custommessage_lambda.function_name}"
  type      = "String"
