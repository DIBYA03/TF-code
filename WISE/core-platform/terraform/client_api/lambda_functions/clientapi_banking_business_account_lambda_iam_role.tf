...
resource "aws_iam_role" "clientapi_banking_business_account_lambda" {
  name = "${module.naming.aws_iam_role}-ban-bus-acc-lambda-${var.api_gw_stage}"
...
resource "aws_iam_role_policy_attachment" "clientapi_banking_business_account_lambda_vpc" {
  role       = "${aws_iam_role.clientapi_banking_business_account_lambda.name}"
  policy_arn = "arn:aws:iam::aws:policy/service-role/AWSLambdaVPCAccessExecutionRole"
}

# inline policy to allow access to segment analytics sqs
resource "aws_iam_role_policy" "clientapi_banking_business_account_segment_analytics_sqs" {
...
resource "aws_iam_role_policy" "clientapi_banking_business_account_lambda_s3_doc_read" {
