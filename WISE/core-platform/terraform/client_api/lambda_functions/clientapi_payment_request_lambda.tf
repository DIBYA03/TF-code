resource "aws_lambda_function" "clientapi_payment_request" {
  function_name = "${module.naming.aws_lambda_function}-pmt-rqt-${var.api_gw_stage}"
  role          = "${aws_iam_role.clientapi_payment_request_lambda.arn}"
  kms_key_arn   = "${aws_kms_key.api_lambda.arn}"
  timeout       = "${var.lambda_timeout}"

  filename         = "../../../cmd/lambda/clientapi/payment/request/lambda.zip"
  source_code_hash = "${base64sha256(file("../../../cmd/lambda/clientapi/payment/request/lambda.zip"))}"
  ...
  tags {
    ...
    Name        = "${module.naming.aws_lambda_function}-pmt-rqt-${var.api_gw_stage}"
    Team        = "${var.team}"
  }
}

# Give permissions for the API Gateway to access the lambda function
resource "aws_lambda_permission" "clientapi_payment_request" {
  ...
  depends_on = [
    "aws_lambda_function.clientapi_payment_request",
  ]
}

resource "aws_cloudwatch_metric_alarm" "clientapi_payment_request_non_critical_errors" {
  alarm_name          = "${module.naming.aws_lambda_function}-pmt-rqt-non-crit-errors-${var.api_gw_stage}"
  ...
  dimensions {
    FunctionName = "${aws_lambda_function.clientapi_payment_request.function_name}"
  }
  ...
  tags {
    ...
    Component   = "${var.component}"
    Environment = "${var.environment_name}"
    Name        = "${module.naming.aws_lambda_function}-pmt-rqt-non-crit-errors-${var.api_gw_stage}"
    Team        = "${var.team}"
  }
}

resource "aws_cloudwatch_metric_alarm" "clientapi_payment_request_critical_errors" {
  alarm_name          = "${module.naming.aws_lambda_function}-pmt-rqt-crit-errors-${var.api_gw_stage}"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "2"
  metric_name         = "Errors"
  namespace           = "AWS/Lambda"
  ...
  tags {
    ...
    Component   = "${var.component}"
    Environment = "${var.environment_name}"
    Name        = "${module.naming.aws_lambda_function}-pmt-rqt-crit-errors-${var.api_gw_stage}"
    Team        = "${var.team}"
  }
}
