...
resource "aws_lambda_function" "clientapi_banking_business_card_block" {
  function_name = "${module.naming.aws_lambda_function}-ban-bus-crd-blk-${var.api_gw_stage}"
  role          = "${data.aws_iam_role.lambda_default.arn}"
  kms_key_arn   = "${aws_kms_key.api_lambda.arn}"
  timeout       = "${var.lambda_timeout}"

  filename         = "../../../cmd/lambda/clientapi/banking/business/card/block/lambda.zip"
  source_code_hash = "${base64sha256(file("../../../cmd/lambda/clientapi/banking/business/card/block/lambda.zip"))}"
  ...
  tags {
    ...
    Name        = "${module.naming.aws_lambda_function}-ban-bus-crd-blk-${var.api_gw_stage}"
    Team        = "${var.team}"
  }
}

# Give permissions for the API Gateway to access the lambda function
resource "aws_lambda_permission" "clientapi_banking_business_card_block" {
  ...
  depends_on = [
    "aws_lambda_function.clientapi_banking_business_card_block",
  ]
}

resource "aws_cloudwatch_metric_alarm" "clientapi_banking_business_card_block_non_critical_errors" {
  alarm_name          = "${module.naming.aws_lambda_function}-ban-bus-crd-blk-non-crit-errors-${var.api_gw_stage}"
  ...
  dimensions {
    FunctionName = "${aws_lambda_function.clientapi_banking_business_card_block.function_name}"
  }
  ...
  tags {
    ...
    Component   = "${var.component}"
    Environment = "${var.environment_name}"
    Name        = "${module.naming.aws_lambda_function}-ban-bus-crd-blk-non-crit-errors-${var.api_gw_stage}"
    Team        = "${var.team}"
  }
}

resource "aws_cloudwatch_metric_alarm" "clientapi_banking_business_card_block_critical_errors" {
  alarm_name          = "${module.naming.aws_lambda_function}-ban-bus-crd-blk-crit-errors-${var.api_gw_stage}"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "2"
  metric_name         = "Errors"
  namespace           = "AWS/Lambda"
  ...
  tags {
    ...
    Component   = "${var.component}"
    Environment = "${var.environment_name}"
    Name        = "${module.naming.aws_lambda_function}-ban-bus-crd-blk-crit-errors-${var.api_gw_stage}"
    Team        = "${var.team}"
  }
}
