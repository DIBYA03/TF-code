...
resource "aws_lambda_function" "clientapi_business_document_lambda" {
  ...
  filename         = "../../../cmd/lambda/clientapi/business/document/lambda.zip"
  source_code_hash = "${base64sha256(file("../../../cmd/lambda/clientapi/business/document/lambda.zip"))}"
...
resource "aws_lambda_permission" "clientapi_business_document_lambda" {
  ...
  depends_on = [
    "aws_lambda_function.clientapi_business_document_lambda",
  ]
}

resource "aws_cloudwatch_metric_alarm" "clientapi_business_document_lambda_non_critical_errors" {
  alarm_name          = "${module.naming.aws_lambda_function}-bus-doc-non-crit-errors-${var.api_gw_stage}"
  ...
  dimensions {
    FunctionName = "${aws_lambda_function.clientapi_business_document_lambda.function_name}"
  }
  ...
  tags {
    ...
    Component   = "${var.component}"
    Environment = "${var.environment_name}"
    Name        = "${module.naming.aws_lambda_function}-bus-doc-non-crit-errors-${var.api_gw_stage}"
    Team        = "${var.team}"
  }
}

resource "aws_cloudwatch_metric_alarm" "clientapi_business_document_lambda_critical_errors" {
  alarm_name          = "${module.naming.aws_lambda_function}-bus-doc-crit-errors-${var.api_gw_stage}"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "2"
  metric_name         = "Errors"
  namespace           = "AWS/Lambda"
  ...
  tags {
    ...
    Component   = "${var.component}"
    Environment = "${var.environment_name}"
    Name        = "${module.naming.aws_lambda_function}-bus-doc-crit-errors-${var.api_gw_stage}"
    Team        = "${var.team}"
  }
}
