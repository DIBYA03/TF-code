...
resource "aws_lambda_function" "clientapi_business_member_lambda" {
  ...
  filename         = "../../../cmd/lambda/clientapi/business/member/lambda.zip"
  source_code_hash = "${base64sha256(file("../../../cmd/lambda/clientapi/business/member/lambda.zip"))}"
...
resource "aws_lambda_permission" "clientapi_business_member_lambda" {
  statement_id  = "AllowExecutionFromAPIGateway"
  action        = "lambda:InvokeFunction"
  function_name = "${aws_lambda_function.clientapi_business_member_lambda.function_name}"

  principal  = "apigateway.amazonaws.com"
  source_arn = "${local.api_gw_arn}/*/*/*"

  depends_on = [
    "aws_lambda_function.clientapi_business_member_lambda",
  ]
}

resource "aws_cloudwatch_metric_alarm" "clientapi_business_member_lambda_non_critical_errors" {
  alarm_name          = "${module.naming.aws_lambda_function}-bus-mem-non-crit-errors-${var.api_gw_stage}"
  ...
  dimensions {
    FunctionName = "${aws_lambda_function.clientapi_business_member_lambda.function_name}"
  }
  ...
  tags {
    ...
    Component   = "${var.component}"
    Environment = "${var.environment_name}"
    Name        = "${module.naming.aws_lambda_function}-bus-mem-non-crit-errors-${var.api_gw_stage}"
    Team        = "${var.team}"
  }
}

resource "aws_cloudwatch_metric_alarm" "clientapi_business_member_lambda_critical_errors" {
  alarm_name          = "${module.naming.aws_lambda_function}-bus-mem-crit-errors-${var.api_gw_stage}"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "2"
  metric_name         = "Errors"
  namespace           = "AWS/Lambda"
  ...
  tags {
    ...
    Component   = "${var.component}"
    Environment = "${var.environment_name}"
    Name        = "${module.naming.aws_lambda_function}-bus-mem-crit-errors-${var.api_gw_stage}"
    Team        = "${var.team}"
  }
}
