...
resource "aws_lambda_function" "clientapi_business_lambda" {
  function_name = "${module.naming.aws_lambda_function}-bus-${var.api_gw_stage}"
  role          = "${aws_iam_role.clientapi_business_lambda.arn}"
  kms_key_arn   = "${aws_kms_key.api_lambda.arn}"
  timeout       = "${var.lambda_timeout}"

  filename         = "../../../cmd/lambda/clientapi/business/lambda.zip"
  source_code_hash = "${base64sha256(file("../../../cmd/lambda/clientapi/business/lambda.zip"))}"
  ...
  tags {
    ...
    Name        = "${module.naming.aws_lambda_function}-bus-${var.api_gw_stage}"
    Team        = "${var.team}"
  }
}

# Give permissions for the API Gateway to access the lambda function
resource "aws_lambda_permission" "clientapi_business_lambda" {
  statement_id  = "AllowExecutionFromAPIGateway"
  action        = "lambda:InvokeFunction"
  function_name = "${aws_lambda_function.clientapi_business_lambda.function_name}"

  principal  = "apigateway.amazonaws.com"
  source_arn = "${local.api_gw_arn}/*/*/*"

  depends_on = [
    "aws_lambda_function.clientapi_business_lambda",
  ]
}

resource "aws_cloudwatch_metric_alarm" "clientapi_business_lambda_non_critical_errors" {
  alarm_name          = "${module.naming.aws_lambda_function}-bus-non-crit-errors-${var.api_gw_stage}"
  ...
  dimensions {
    FunctionName = "${aws_lambda_function.clientapi_business_lambda.function_name}"
  }
  ...
  tags {
    ...
    Component   = "${var.component}"
    Environment = "${var.environment_name}"
    Name        = "${module.naming.aws_lambda_function}-bus-non-crit-errors-${var.api_gw_stage}"
    Team        = "${var.team}"
  }
}

resource "aws_cloudwatch_metric_alarm" "clientapi_business_lambda_critical_errors" {
  alarm_name          = "${module.naming.aws_lambda_function}-bus-crit-errors-${var.api_gw_stage}"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "2"
  metric_name         = "Errors"
  namespace           = "AWS/Lambda"
  ...
  tags {
    ...
    Component   = "${var.component}"
    Environment = "${var.environment_name}"
    Name        = "${module.naming.aws_lambda_function}-bus-crit-errors-${var.api_gw_stage}"
    Team        = "${var.team}"
  }
}
