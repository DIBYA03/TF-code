resource "aws_lambda_function" "business_account_closure_lambda" {
  ...
  filename         = "../../../cmd/lambda/clientapi/business/accountclosure/lambda.zip"
  source_code_hash = "${base64sha256(file("../../../cmd/lambda/clientapi/business/accountclosure/lambda.zip"))}"
...
resource "aws_cloudwatch_metric_alarm" "business_account_closure_lambda_non_critical_errors" {
  alarm_name          = "${module.naming.aws_lambda_function}-bus-acc-cls-non-crit-errors-${var.api_gw_stage}"
  ...
  tags {
    ...
    Component   = "${var.component}"
    Environment = "${var.environment_name}"
    Name        = "${module.naming.aws_lambda_function}-bus-acc-cls-non-crit-errors-${var.api_gw_stage}"
    Team        = "${var.team}"
  }
}

resource "aws_cloudwatch_metric_alarm" "business_account_closure_lambda_critical_errors" {
  alarm_name          = "${module.naming.aws_lambda_function}-bus-acc-cls-crit-errors-${var.api_gw_stage}"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "2"
  metric_name         = "Errors"
  namespace           = "AWS/Lambda"
  ...
  tags {
    ...
    Component   = "${var.component}"
    Environment = "${var.environment_name}"
    Name        = "${module.naming.aws_lambda_function}-bus-acc-cls-crit-errors-${var.api_gw_stage}"
    Team        = "${var.team}"
  }
}
