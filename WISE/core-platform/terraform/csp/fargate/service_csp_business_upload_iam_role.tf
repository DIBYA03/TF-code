resource "aws_iam_role" "csp_business_upload_execution_role" {
  name = "${module.naming.aws_iam_role}-csp-business-upload-execution-role"
...
resource "aws_iam_role_policy" "csp_business_upload_s3_doc_read_write" {
  name = "${module.naming.aws_iam_role_policy}-s3-read-write"
  role = "${aws_iam_role.csp_business_upload_execution_role.id}"
...
resource "aws_iam_role_policy" "csp_business_upload_segment_analytics_sqs" {
...
resource "aws_iam_role_policy" "csp_business_upload_execution_role" {
...
resource "aws_iam_role_policy" "business_upload_sqs" {
  name = "${module.naming.aws_iam_role_policy}-business-upload-sqs"
  role = "${aws_iam_role.csp_business_upload_execution_role.name}"
  ...
  policy = <<EOF
{
  ...
  "Statement": [
    {
      "Sid": "AllowAccessToDocumentUploadSQSQueue",
      "Effect": "Allow",
      "Action": [
        "kms:Decrypt",
...
resource "aws_iam_role_policy" "business_upload_ssm" {
