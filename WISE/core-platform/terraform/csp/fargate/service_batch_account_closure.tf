resource "aws_cloudwatch_log_group" "batch_account_closure" {
  name              = "/ecs/${module.naming.aws_cloudwatch_log_group}-batch-account-closure"
...
data "template_file" "batch_account_closure" {
  template = "${file("definitions/tasks/batch_account_closure.json")}"

  vars {
    aws_log_group = "${module.naming.aws_cloudwatch_log_group}-batch-account-closure"
    ...
    ecr_image      = "${var.batch_account_closure_image}"
    ecr_image_tag  = "${var.batch_account_closure_image_tag}"
    fargate_cpu    = "${var.batch_account_closure_cpu}"
    fargate_memory = "${var.batch_account_closure_mem}"
    name           = "${var.batch_account_closure_name}"
    ...
    wise_refund_clearing_linked_account_id     = "${data.aws_ssm_parameter.wise_refund_clearing_linked_account_id.arn}"
...
resource "aws_ecs_task_definition" "batch_account_closure" {
  family             = "${module.naming.aws_ecs_task_definition}-batch-account-closure"
  execution_role_arn = "${aws_iam_role.batch_account_closure_execution_role.arn}"       # Service management
  task_role_arn      = "${aws_iam_role.batch_account_closure_execution_role.arn}"       # To run AWS services

  network_mode             = "awsvpc"
  requires_compatibilities = ["FARGATE"]
  cpu                      = "${var.batch_account_closure_cpu}"
  memory                   = "${var.batch_account_closure_mem}"
  container_definitions    = "${data.template_file.batch_account_closure.rendered}"

  tags {
    Application = "${var.application}"
    Component   = "${var.component}"
    Environment = "${var.environment_name}"
    Name        = "${module.naming.aws_ecs_task_definition}-batch-account-closure"
    Team        = "${var.team}"
  }
}
