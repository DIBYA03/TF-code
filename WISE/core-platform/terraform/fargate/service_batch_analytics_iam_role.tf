resource "aws_iam_role" "batch_analytics_execution_role" {
  name = "${module.naming.aws_iam_role}-batch-analytics-execution-role"
...
resource "aws_iam_role_policy" "batch_analytics_execution_role" {
  name = "${module.naming.aws_iam_policy}-default-execution"
  role = "${aws_iam_role.batch_analytics_execution_role.id}"
