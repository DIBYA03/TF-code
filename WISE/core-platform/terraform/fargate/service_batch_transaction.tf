resource "aws_cloudwatch_log_group" "batch_transaction" {
  name              = "/ecs/${module.naming.aws_cloudwatch_log_group}-batch-transaction"
...
data "template_file" "batch_transaction" {
  template = "${file("definitions/tasks/batch_transaction.json")}"

  vars {
    aws_log_group = "${module.naming.aws_cloudwatch_log_group}-batch-transaction"
    ...
    ecr_image      = "${var.batch_transaction_image}"
    ecr_image_tag  = "${var.batch_transaction_image_tag}"
    fargate_cpu    = "${var.batch_transaction_cpu}"
    fargate_memory = "${var.batch_transaction_mem}"
    name           = "${var.batch_transaction_name}"
...
resource "aws_ecs_task_definition" "batch_transaction" {
  family             = "${module.naming.aws_ecs_task_definition}-batch-transaction"
  execution_role_arn = "${aws_iam_role.batch_transaction_execution_role.arn}"       # Service management
  task_role_arn      = "${aws_iam_role.batch_transaction_execution_role.arn}"       # To run AWS services

  network_mode             = "awsvpc"
  requires_compatibilities = ["FARGATE"]
  cpu                      = "${var.batch_transaction_cpu}"
  memory                   = "${var.batch_transaction_mem}"
  container_definitions    = "${data.template_file.batch_transaction.rendered}"

  tags {
    Application = "${var.application}"
    Component   = "${var.component}"
    Environment = "${var.environment_name}"
    Name        = "${module.naming.aws_ecs_task_definition}-batch-transaction"
    Team        = "${var.team}"
  }
}
