resource "aws_iam_role" "batch_monthly_interest_execution_role" {
  name = "${module.naming.aws_iam_role}-batch-monthly-interest-execution-role"
...
resource "aws_iam_role_policy" "batch_monthly_interest_execution_role" {
  name = "${module.naming.aws_iam_policy}-default-execution"
  role = "${aws_iam_role.batch_monthly_interest_execution_role.id}"
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": [
        ...
        "${data.aws_ssm_parameter.wise_clearing_linked_account_id.arn}"
      ]
    }
  ]
}
