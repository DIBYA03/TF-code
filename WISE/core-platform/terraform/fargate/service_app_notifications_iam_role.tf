resource "aws_iam_role" "app_notifications_execution_role" {
  name = "${module.naming.aws_iam_role}-app-notifications-execution-role"
...
resource "aws_iam_role_policy" "app_notifications_execution_role" {
  name = "${module.naming.aws_iam_policy}-default-execution"
  role = "${aws_iam_role.app_notifications_execution_role.id}"
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": [
        ...
        "${data.aws_ssm_parameter.firebase_config.arn}",
    ...
    },
    {
      ...
      "Resource": [
        ...
        "${data.aws_sqs_queue.csp_review.arn}"
      ]
    },
    {
      "Action": [
      ...
      "Effect": "Allow",
      "Resource": [
        "${data.aws_kms_alias.transactions_kinesis.target_key_arn}",
        "arn:aws:firehose:${var.aws_region}:${data.aws_caller_identity.account.account_id}:deliverystream/${var.txn_kinesis_name}"
      ]
    }
  ]
}
...
resource "aws_iam_role_policy" "app_notification_s3_read_write" {
