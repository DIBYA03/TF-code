resource "aws_cloudwatch_log_group" "batch_account" {
  name              = "/ecs/${module.naming.aws_cloudwatch_log_group}-batch-account"
...
data "template_file" "batch_account" {
  template = "${file("definitions/tasks/batch_account.json")}"

  vars {
    aws_log_group = "${module.naming.aws_cloudwatch_log_group}-batch-account"
    ...
    ecr_image      = "${var.batch_account_image}"
    ecr_image_tag  = "${var.batch_account_image_tag}"
    fargate_cpu    = "${var.batch_account_cpu}"
    fargate_memory = "${var.batch_account_mem}"
...
resource "aws_ecs_task_definition" "batch_account" {
  family             = "${module.naming.aws_ecs_task_definition}-batch-account"
  execution_role_arn = "${aws_iam_role.batch_account_execution_role.arn}"       # Service management
  task_role_arn      = "${aws_iam_role.batch_account_execution_role.arn}"       # To run AWS services

  network_mode             = "awsvpc"
  requires_compatibilities = ["FARGATE"]
  cpu                      = "${var.batch_account_cpu}"
  memory                   = "${var.batch_account_mem}"
  container_definitions    = "${data.template_file.batch_account.rendered}"

  tags {
    Application = "${var.application}"
    Component   = "${var.component}"
    Environment = "${var.environment_name}"
    Name        = "${module.naming.aws_ecs_task_definition}-batch-account"
    Team        = "${var.team}"
  }
}
