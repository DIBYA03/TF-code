resource "aws_iam_role" "batch_monitor_execution_role" {
  name = "${module.naming.aws_iam_role}-batch-monitor-execution-role"
...
resource "aws_iam_role_policy" "batch_monitor_execution_role" {
  name = "${module.naming.aws_iam_policy}-default-execution"
  role = "${aws_iam_role.batch_monitor_execution_role.id}"
