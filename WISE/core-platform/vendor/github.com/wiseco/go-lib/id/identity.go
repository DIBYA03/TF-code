package id

import (
	"database/sql/driver"
	"encoding/json"
	"errors"
	"fmt"
	"strings"

	"github.com/google/uuid"
)

/*
 * Identity prefix + uuid will be used internally when passed between services and
 * removed when passing to/from the database.
 */
type IdentityID uuid.UUID

func (id IdentityID) Prefix() IDPrefix {
	return IDPrefixIdentity
}

func (id IdentityID) String() string {
	return string(IDPrefixIdentity) + id.UUIDString()
}

func (id IdentityID) UUIDString() string {
	return uuid.UUID(id).String()
}

func (id IdentityID) IsZero() bool {
	return uuid.UUID(id) == uuid.Nil
}

// Returns empty string when zero for omitempty
func (id IdentityID) JSONString() string {
	if id.IsZero() {
		return ""
	}

	return id.String()
}

func (id IdentityID) MarshalJSON() ([]byte, error) {
	return json.Marshal(id.String())
}

func (id *IdentityID) UnmarshalJSON(data []byte) error {
	var s string
	if err := json.Unmarshal(data, &s); err != nil {
		return err
	}

	uid, err := ParseIdentityID(s)
	if err != nil {
		return err
	}

	*id = uid
	return nil
}

// SQL value marshaller
func (id IdentityID) Value() (driver.Value, error) {
	return id.UUIDString(), nil
}

// SQL scanner
func (id *IdentityID) Scan(value interface{}) error {
	if value == nil {
		*id = IdentityID{}
		return nil
	}

	val, ok := value.([]byte)
	if !ok {
		return fmt.Errorf("Unable to scan value %v", value)
	}

	uid, err := uuid.Parse(string(val))
	if err != nil {
		return err
	}

	*id = IdentityID(uid)
	return nil
}

func ParseIdentityID(id string) (IdentityID, error) {
	// Return nil id on empty string
	if id == "" {
		return IdentityID{}, nil
	}

	// Check prefix
	if !strings.HasPrefix(id, string(IDPrefixIdentity)) {
		return IdentityID{}, errors.New("invalid identity id prefix")
	}

	// Validate UUID
	uid, err := uuid.Parse(strings.TrimPrefix(id, string(IDPrefixIdentity)))
	if err != nil {
		return IdentityID{}, err
	}

	return IdentityID(uid), nil
}
