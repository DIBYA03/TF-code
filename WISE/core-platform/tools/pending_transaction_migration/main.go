...
func main() {
	...
	for {
		...
		for _, t := range transactions {
			go func(t transaction.BusinessPendingTransaction) {
				defer wg.Done()
...
func getAllTransactions(offset, limit int) ([]transaction.BusinessPendingTransaction, error) {
	transactions := []transaction.BusinessPendingTransaction{}
	...
	WHERE business_pending_transaction.transaction_date > $1
	ORDER BY business_pending_transaction.transaction_date ASC
	OFFSET $2
	LIMIT $3`
...
func processTransaction(txnClient grpcBankTxn.TransactionServiceClient, t transaction.BusinessPendingTransaction, businessDetail *BusinessDetails) {
	...
	case transaction.TransactionTypeTransfer:
		...
		if err != nil {
			log.Println("Error processing transfer transaction", err)
			return
		}
	...
	case transaction.TransactionTypeVisaCredit:
		...
		if err != nil {
			log.Println("Error processing debit pull transaction", err)
		}
	...
	case transaction.TransactionTypeOther:
		...
		case transaction.TransactionCodeTypeAuthApproved:
			...
			u.Status = transaction.TransactionStatusCardAuthorized
			u.TransactionType = transaction.TransactionTypePurchase
		case transaction.TransactionCodeTypeAuthDeclined:
			...
			u.Status = transaction.TransactionStatusCardAuthDeclined
			u.TransactionType = transaction.TransactionTypePurchase
		default:
			log.Println("Unhandled transaction type", t.TransactionType, t.CodeType)
		}
	default:
		log.Println("Unhandled transaction type", t.TransactionType)
		return
	}
	...
	if t.CardID != nil && len(*t.CardID) > 0 {
		...
		if err != nil {
			log.Printf("Error parsing debit card id: %v", err)
			return
		}
	...
	if t.MoneyTransferID != nil && len(*t.MoneyTransferID) > 0 {
		...
		if err != nil {
			log.Printf("Error parsing bank transfer id: %v", err)
			return
		}
	...
	if !btID.IsZero() {
		...
		if err != nil {
			log.Printf("Error getting bank transfer: %v", err)
			return
		}
	}
	...
	if t.MoneyRequestID != nil && len(*t.MoneyRequestID) > 0 {
		...
		if err != nil {
			log.Printf("Error parsing payment request id: %v", err)
			return
		}
	...
	if !ok || status == grpcTxn.BankTransactionStatus_BTS_UNSPECIFIED {
		...
		if !ok || status == grpcTxn.BankTransactionStatus_BTS_UNSPECIFIED {
			...
			case
				banking.MoneyTransferStatusPullFailed,
				banking.MoneyTransferStatusPullRefunded,
				banking.MoneyTransferStatusPullReview,
				banking.MoneyTransferStatusPushRefunded,
				banking.MoneyTransferStatusPushReview:
				status = grpcTxn.BankTransactionStatus_BTS_BANK_TRANSFER_ERROR
			default:
				status = grpcTxn.BankTransactionStatus_BTS_BANK_PROCESSING
			}
		}
	}

	legacyTxnType := u.TransactionType
	category, ok := transaction.TransactionTypeToCategoryProto[u.TransactionType]
	if !ok || category == grpcTxn.BankTransactionCategory_BTC_UNSPECIFIED {
		legacyTxnType = t.TransactionType
		category, ok = transaction.TransactionTypeToCategoryProto[t.TransactionType]
		if !ok || category == grpcTxn.BankTransactionCategory_BTC_UNSPECIFIED {
			log.Println("invalid or unspecified transaction category type")
			return
		}
	}
	...
	if t.TransactionSubtype != nil {
		...
		if !ok || txnType == grpcTxn.BankTransactionType_BTT_UNSPECIFIED {
			...
			if !ok || txnType == grpcTxn.BankTransactionType_BTT_UNSPECIFIED {
				// Check code type
				switch t.CodeType {
				case transaction.TransactionCodeTypeAuthApproved, transaction.TransactionCodeTypeAuthDeclined:
					subtype = transaction.TransactionSubtypeCardPurchaseDebit
					txnType = grpcTxn.BankTransactionType_BTT_CARD_PURCHASE_DEBIT
				default:
	...
	case grpcTxn.BankTransactionType_BTT_UNSPECIFIED:
		if acc.UsageType == business.UsageTypeClearing {
			if mt == nil {
				log.Println("clearing account transaction missing money transfer")
				return
			}
	...
	var created *timestamp.Timestamp
	if t.Created != nil {
	...
	req := &grpcBankTxn.UpsertTransactionRequest{
		...
		Created:                created,
		LegacyType:             string(legacyTxnType),
	...
	if t.CardTransaction != nil {
		...
		query := "SELECT * FROM business_card_pending_transaction where id = $1 LIMIT 1"
		...
		if err == nil {
			...
			if !ok {
				log.Println("invalid card network", card.TransactionNetwork)
				return
			}
	...
	if t.HoldTransaction != nil {
		...
		query := "SELECT * FROM business_hold_pending_transaction where id = $1 LIMIT 1"
	...
	if err != nil {
		log.Println("Transaction service error:", err, "TXN ID:", t.ID, "TXN Status:", t.Status)
	} else {
		log.Println("Transaction service success:", resp.Transaction.Id)
	}
}

func processTransferTransactions(t transaction.BusinessPendingTransaction,
	transactionDetail *TransactionDetails, businessDetail *BusinessDetails) (*TransactionUpdate, error) {

	if t.CodeType == transaction.TransactionCodeTypeCreditInProcess {
...
func processMoneyRequestCreditTransaction(t transaction.BusinessPendingTransaction,
	transactionDetail *TransactionDetails, businessDetail *BusinessDetails) (*TransactionUpdate, error) {
...
func processInterestCreditTransaction(t transaction.BusinessPendingTransaction,
	transactionDetail *TransactionDetails, businessDetail *BusinessDetails) (*TransactionUpdate, error) {
	u := TransactionUpdate{}
...
func processTransferCreditTransaction(t transaction.BusinessPendingTransaction,
	transactionDetail *TransactionDetails, businessDetail *BusinessDetails) (*TransactionUpdate, error) {
	u := TransactionUpdate{}
...
func processInstantPayDebitTransaction(t transaction.BusinessPendingTransaction,
	transactionDetail *TransactionDetails, businessDetail *BusinessDetails) (*TransactionUpdate, error) {
	u := TransactionUpdate{}
...
func processDepositCreditTransaction(t transaction.BusinessPendingTransaction,
	transactionDetail *TransactionDetails, businessDetail *BusinessDetails) (*TransactionUpdate, error) {
	u := TransactionUpdate{}
	...
	senderName := shared.StringValue(transaction.GetOriginAccountHolder(transaction.TransactionTypeDeposit, *t.BankTransactionDesc))
...
func processAccountOriginationTransaction(t transaction.BusinessPendingTransaction,
	transactionDetail *TransactionDetails, businessDetail *BusinessDetails) (*TransactionUpdate, error) {
	u := TransactionUpdate{}
...
func processACHTransaction(t transaction.BusinessPendingTransaction,
	transactionDetail *TransactionDetails, businessDetail *BusinessDetails) (*TransactionUpdate, error) {
...
func processACHExternalDebitTransaction(t transaction.BusinessPendingTransaction,
	transactionDetail *TransactionDetails, businessDetail *BusinessDetails) (*TransactionUpdate, error) {
	u := TransactionUpdate{}
...
func processACHInternalDebitTransaction(t transaction.BusinessPendingTransaction,
	transactionDetail *TransactionDetails, businessDetail *BusinessDetails) (*TransactionUpdate, error) {
	u := TransactionUpdate{}
...
func processACHCreditTransaction(t transaction.BusinessPendingTransaction,
	transactionDetail *TransactionDetails, businessDetail *BusinessDetails) (*TransactionUpdate, error) {
	u := TransactionUpdate{}
...
func processMerchantRefundDebitTransaction(t transaction.BusinessPendingTransaction,
	transactionDetail *TransactionDetails, businessDetail *BusinessDetails) (*TransactionUpdate, error) {
	u := TransactionUpdate{}
...
func processATMTransaction(t transaction.BusinessPendingTransaction,
	transactionDetail *TransactionDetails, businessDetail *BusinessDetails) (*TransactionUpdate, error) {
	u := TransactionUpdate{}
...
func processCardPurchaseDebitTransaction(t transaction.BusinessPendingTransaction,
	transactionDetail *TransactionDetails, businessDetail *BusinessDetails) (*TransactionUpdate, error) {
	u := TransactionUpdate{}
...
func processTransferDebitTransaction(t transaction.BusinessPendingTransaction,
	transactionDetail *TransactionDetails, businessDetail *BusinessDetails) (*TransactionUpdate, error) {
	u := TransactionUpdate{}
...
func updateTransaction(ID shared.PendingTransactionID, u *TransactionUpdate) error {
	var columns []string
	...
	query := fmt.Sprintf(
		"UPDATE business_pending_transaction SET %s WHERE id = '%s'",
